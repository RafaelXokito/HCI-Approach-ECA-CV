...
The integration of empathy in Human-Computer Interaction (HCI) is essential for enhancing user experiences. Current HCI systems often overlook users' emotional states, limiting interaction quality. This study presents a novel approach to overcome this limitation by incorporating Multimodal Emotion Recognition (MER) and empathic conversational agents into HCI. The proposed architecture leverages deep learning techniques, including Computer Vision (CV), Sentiment Analysis (SA), and Large Language Models (LLMs), to continuously understand and respond to user emotions empathically. The system fuses multiple data streams—textual, auditory, and visual—to accurately detect emotions. This paper highlights the advantages of this multimodal approach over traditional unimodal systems in recognizing complex human emotions. The findings include a comprehensive guide on the impact of deep learning in HCI, a literature review on Emotion Recognition (ER) and conversational agents, and the proposal of an HCI architecture integrating MER with an empathic conversational agent. This research contributes significantly to the field of HCI by providing a detailed roadmap for developing more realistic and meaningful human-computer interactions.

\keywords{Human-Computer Interaction  \and Multimodal Emotion Recognition \and Empathic Conversational Agents}
\end{abstract}
%
%
%
...
Artificial Intelligence (AI) encompasses several techniques and methodologies aimed at enabling machines to perform tasks that typically require human intelligence, whereas deep learning stands out as a specialized approach relying on Artificial Neural Networks (ANNs) to process unstructured data (including images, voice, videos, and text, among others). ER, being a recent application of deep learning, involves detecting human emotions, generally through singular modalities such as facial features, gestures, poses, speech, and text captured through continuous interactions with the user \cite{alrowais_modified_2023}. On the other hand, conversational agents are designed to simulate human-like conversation and engage in interactions with users through natural language using several techniques, including natural language processing (NLP), and deep learning with the recent advances in Large Language Models (LLMs), to understand user input, interpret context, and generate appropriate responses.

Due to the potential of Multimodal Emotion Recognition (MER) and conversational agents, individually, and the benefits provided when integrated with HCI, this study offers a guide covering ER modalities and key design and functionality aspects of a conversational agent. Lastly, proposing an HCI approach to ensure more realistic and meaningful interactions by leveraging HCI in conjunction with a MER fusion technique and an empathic conversational agent.
...
\begin{itemize}
	...
	\item Performed a literature review regarding ER, fusion methods, and conversational agents;
	\item Proposal of an architecture of an HCI approach aided with MER, through Computer Vision (CV), Sentiment Analysis (SA) a fusion method, and emphatic conversational agent powered with a LLM.
\end{itemize}
...
% SA & SER
As previously detailed, the system could gather the visual and speech or textual data resulting from either in real-time or in video. Hence, the MER module analyzes in parallel all the present modalities, starting with textual data being analyzed by an SA model to obtain the sentiment conveyed in the textual input, following preprocessing, feature extraction, and classification done with a 1D CNN classifier \cite{hung_beyond_2023}. On the other hand, the speech input can be firstly transcribed with automatic speech recognition algorithms, namely HMM-SVM hybrid model \cite{malik_automatic_2021}, to be passed on to the SA model as text, while also being analyzed by the SER module composed of a CNN that analyzes the speech spectrogram (which represents the strength or loudness of a signal over time at different frequencies in a particular waveform) to extract the emotion conveyed in the speech \cite{Badshah2017}.
...
On the other hand, the visual data, obtained from CV, the user's macro-facial expressions and body pose are passed on to a Facial Emotion Recognition (FER) network and Body Emotion Recognition (BER) to further extract the emotional data conveyed in both input types.
...
This research explores the integration of Multimodal Emotion Recognition (MER) and empathic conversational agents in Human-Computer Interaction (HCI). The study contributes to HCI by proposing an architecture that synergizes multiple modalities—textual, auditory, and visual—to detect user emotions accurately and respond empathically. The proposed solution demonstrates enhanced accuracy and robustness in emotion detection compared to traditional unimodal approaches, thanks to the fusion of diverse data streams.

Future work should focus on refining this architecture, particularly in improving the accuracy of emotion detection in varied contexts and across diverse populations. Additionally, exploring the scalability of the system in different applications, such as education and healthcare, would be valuable. Further research could also investigate the ethical implications and privacy concerns associated with emotion recognition technologies.

In conclusion, this study marks a significant advancement in HCI by proposing a novel, multimodal approach to empathic interactions. The integration of MER and empathic conversational agents holds great potential for creating more authentic and meaningful human-computer interactions, which can be adapted to various fields for enhanced user experiences.
