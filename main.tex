...
Empathy is a fundamental trait in ensuring realistic, natural, and meaningful interactions in the field of HCI, therefore this section will delve into the proposed HCI approach architecture consisting of six modules as detailed in Figure \ref{fig:approachArchitecture}. Early emotion recognition mainly focused on text or facial features alone, however, relying on any of these fields separately is not sufficient to extract the sentiments expressed by humans, hence leading to a multimodal emotion recognition, where multiple modalities (text, audio, and video) are analyzed simultaneously to better capture human emotions,  moreover using a fusion model to aggregate each the emotion obtained from each module to a final emotion \cite{zhu_multimodal_2023}.

% "relying on any of these fields separately is not sufficient to extract the sentiments expressed by humans" -> Eu não diria que não seja suficiente assim, eu diria, que devido aos desafios que estão a ser encarados nestes métodos singularmente (citação necessária), então propomos uma arquitetura que ambiciona contribuir para a solução desse problema. E depois ", hence leading to a multimodal emotion recognition".
...
In the proposed architecture, the emotion detection process then begins with textual or audio data captured in continuously and real time from interactions with the CA. When present, the textual data is passed on to the SA network to obtain the sentiment conveyed in the textual input. Nowadays, emotion detection regarding textual data is shifting from the early key-word comparisons to ML (SVM, Naive Bayes, Maximum Entropy) and DL algorithms (CNN, ensemble of CNNs) and NNs (LSTM, DNNs), and even hybrid approaches \cite{hung_beyond_2023}, which any could be chosen as the SA network. % What is SA network??? The two paragraphs bellow could be merged in this paragraph and then talk about the fusion.

Moreover, the speech input could be transcribed using a Speech-To-Text technique and be analyzed as well by the SA network whereas the speech itself could go through the SER network to analyze the speech's para-linguistic information e.g. intonation, duration, prosody, pitch, rhythm among others.

On the other hand, the video input data, obtained from the CV algorithm that would capture,  also continuously and in real time, the user's facial expressions and body pose are passed on to a FER network and BER to further extract the emotional data conveyed in both input types.
