...
% This paragraph seems orphan
%Deep learning's implementation in supervised models includes data collection and preprocessing, training, optimization, and testing \cite{Schmidhuber2015}. In recent years, Facial Emotion Recognition (FER) systems have shown the efficacy of Artificial Neural Networks (ANNs) over traditional machine learning methods \cite{Giannopoulos2018}~\cite{Cirneanu2023}~\cite{Zhao2015}.

%In the context of computer vision, neural networks have been particularly successful in image classification tasks, including face identification and facial emotion recognition. There are many popular neural network architectures that were trained to meet specific requirements, to use them to another similar task transfer learning can be used. This approach involves adjusting and reusing layers of a neural network trained on one dataset to work with a new dataset \cite{Cirneanu2023}~\cite{Zhao2015}.
